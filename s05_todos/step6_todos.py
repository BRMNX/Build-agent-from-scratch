...
"""
=======================
s05 new content : todos
=======================
"""

"""
===================================================================================
    + 维护一个 TodoList(TodoManager对象)，实际上是list[dict]类型
    + list里的每一项都是一个todo(dict类型)，形如
        {"content": "Read lyrics.txt", "status": "completed"},
      或者
        {"content": "Create a file called hello.py", "status": "in_progress"}
        {"content": "Delete the file called hello.py", "status": "waiting"}
===================================================================================
"""
class TodoManager:
    def __init__(self):
...
def agent_loop(messages : list):
    ...
    while True:
        ...
        """
        如果单纯只看多少轮没用过todo，而不看todo里有什么，就会做出多余的提醒，比如：
        - 这一轮没有做todo_write，但是所有任务都completed，此时没必要提醒模型
        用has_work_to_do判断模型是否真的还需要做计划：
        1. 统计所有todo的状态，如果存在状态不为completed的todo，设为True
        2. 注意边界情况，当模型从未做过todo_write时，TODO_LSIT为空，此时所有todo的状态都不为completed，我们需要提醒模型，设为True
        3. TODO_LIST非空且全完成，不要提醒模型，设为False
        """
        ...
        if rounds_since_todo >= 3 and has_work_to_do:
            ...
            rounds_since_todo = 0
            # 这里的重置为0其实是必要且安全的一种做法
            # 我们只是提醒模型做计划，不代表模型一定会做计划
            # 如果模型下一轮做了计划，那么 used_todo = True，rounds自然重置为0
            # 如果模型一直不做计划，那么rounds持续增长，每一轮都会提醒模型，这是不希望看到的
